...
class WhiteBlackApp:
    ...
    def __init__(self, url, domain):
       ...
       # function which reads through the blacklists and determines if url is contained within
    def blacklist_test(self):
        ...
        for blacklist_url_item in self.blacklist_url:  # for each list item in blacklist of urls
            # if item at position 0 equals the url set verdict to True and return ("suspicious")
    ...
    def run(self):
        ...
        if verdict_bool:  # if the returned value is true i.e. the url
        ...
        # If it is return "suspicious" verdict
    ...
    def similar_domains(self):
        ...
        for item in matches:  # loop through the list of similar domains 'get_close_matches' generated
            ...
            if item != self.domain:  # if the domain doesn't match the similar domain, return "suspicious" verdict
        ...
        for item in split_list:
            ...
            # if there are any close matches (i.e. length of the resulting list > 0) return "suspicious" verdict
            if len(matches) > 0:
        ...
        return verdict  # return verdict as False (unknown if "suspicious" or not)
    ...
    def whitelist_test(self):
        ...
        for whitelist_url_item in self.whitelist_url:  # for each list item in whitelist of urls
            ...
            # if item at position 0 equals the url set verdict to True and return ("not suspicious")
            if whitelist_url_item[0] == self.url:
                verdict = True
                print("[*]  URL in whitelist: " + colored(whitelist_url_item[0], 'red') + "  [*]")  # UI
                return verdict

            # check if the url's domain is that of a top domain. If that is the case set verdict to True and return ("not suspicious")
